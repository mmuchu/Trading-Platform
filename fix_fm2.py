...
path = os.path.join(base, "feed_monitor.py")
content = '''"""
Gate 1 — Feed Monitor.
...
print(f"[OK] written {os.path.getsize(path)} bytes")

tp = os.path.dirname(os.path.dirname(os.path.abspath(path)))
if tp not in sys.path:
    sys.path.insert(0, tp)
for m in list(sys.modules):
    if "risk_guard" in m or "feed_monitor" in m:
        del sys.modules[m]
from risk_guard.feed_monitor import FeedMonitor
params = list(inspect.signature(FeedMonitor.__init__).parameters.keys())
print(f"Params: {params}")
if params == ["self", "bus", "stale_threshold_sec", "gap_threshold_sec"]:
    print("[VERIFIED] feed_monitor.py correct!")
else:
    print("[FAIL] wrong params!")
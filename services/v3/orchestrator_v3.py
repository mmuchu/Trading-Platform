...
class V3Orchestrator:
    """
    Production orchestrator for the v3 hybrid event-bus architecture.

    Pipeline (v3.1 with 5-gate risk):
        TICK → Strategy → [SIGNAL → RiskGuard(5 gates)] → Execution → FILL → Analytics
                                ↓                              ↑
                           SL/TP Manager ←─ TICK (continuous monitoring)

    Usage:
        orch = V3Orchestrator(mode="sim")
        await orch.start()       # blocks until shutdown
    """
    ...
    def __init__(self, mode: str = "sim") -> None:
        ...
        # 5-gate risk guard (wraps execution with pre-trade checks)
        self.risk_guard = PortfolioRiskGuard(
            bus=self.bus,
            execution_service=self.execution,
        )

        # Wire risk guard into execution service
    ...
    async def start(self) -> None:
        """Full startup — wires bus, starts services, blocks until shutdown."""
        self._running = True
        logger.info("=" * 55)
        ...
        # Start risk guard first (subscribes to TICK and FILL)
        await self.risk_guard.start()

        # Wire the event bus pipeline:
        #   TICK → strategy, analytics
        #   SIGNAL → execution (now uses risk_guard internally)
        #   FILL → analytics (also consumed by risk_guard)
        ...
        # Start market data feed (this blocks)
